...
urlpatterns = [
    ...
    url(r'^results/(?P<year>[0-9]{4})/(?P<place>\w+)/(?P<chamber_name>\w+)/$', views.party_election_list),
    url(r'^results/(?P<year>[0-9]{4})/(?P<place>\w+)/(?P<chamber_name>\w+)/(?P<name>\w+)/$', views.party_election_detail),
